...
function usage {
  ...
  echo "  -o/--outdir       output directory for trimmed fastq files (need the full path)"
  echo "  -s/--suffix       suffix denoting forward reads in the input directory, e.g. _1.fastq"
...
case $key in
  ...
  shift
  ;;
  -s|--suffix) # suffix of forward reads (e.g _1.fastq for SRR3362713_1.fastq)
  SUFFIX="$2"
  shift
  ;;
  --ilmnclip) # ILLUMINACLIP argument.  /path/adapter.fa:seedmismatches:palindromeclipthreshold:simpleclipthreshold (e.g. /path:2:30:10)
  ILMNCLIP="$2"
  shift
  ;;
...
do
  ...
  java -jar '/mnt/disks/disk-2/Tools/Trimmomatic-0.36/trimmomatic-0.36.jar' PE -threads $THREADS -basein $f -baseout $OUTPATH ILLUMINACLIP:$ILMNCLIP HEADCROP:$HEADCROP SLIDINGWINDOW:$SLIDINGWINDOW
done
