...
function usage {
  ...
  echo
  echo "Runs FastQC on all .fq files in a specified input directory and writes .html reports to an output directory. Also compiles a MultiQC report in the output directory. Can run as many files in parallel as there are threads."
  echo
  echo "  -i/--indir    input directory of fastq files"
  echo "  -o/--outdir   output directory for html reports"
  echo "  -t/--threads  number of threads"
  echo "  Version control: Nick Geraci September 14, 2017"
  echo
  exit 1
}
