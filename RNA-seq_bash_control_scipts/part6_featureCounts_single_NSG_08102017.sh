...
function usage {
  ...
  echo
  echo "Runs featureCounts on sorted bam files and writes countsall.txt and countsall.txt.summary reports to an output directory"
  echo
...
if [[ $M = 1 ]]
then
/mnt/disks/disk-2/Tools/subread-1.5.3-Linux-x86_64/bin/featureCounts -s $STRANDED -T $THREADS -M -a /mnt/disks/disk-2/Tools/Reference_Genomes/Homo_sapiens/UCSC/hg38/Annotation/Genes.gencode/genes.gtf -F GTF -t exon -g gene_id -o $OUTPATH $INFILES
else
/mnt/disks/disk-2/Tools/subread-1.5.3-Linux-x86_64/bin/featureCounts -s $STRANDED -T $THREADS -a /mnt/disks/disk-2/Tools/Reference_Genomes/Homo_sapiens/UCSC/hg38/Annotation/Genes.gencode/genes.gtf -F GTF -t exon -g gene_id -o $OUTPATH $INFILES
fi
